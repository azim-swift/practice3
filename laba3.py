...
, чем количество нулевых  элементов в четных столбцах в области 1, то поменять в В симметрично области 2 и 3 местами"""
for i in range(0,(N//4)-1): # ищем в С количество нулевых элементов в нечетных столбцах в области 4
    wall += 1
    for j in range(0+wall , (N//2)-wall ):
        
        if i < j  and i < ((N//2) - 1) - j and C[i][j]==0 and j % 2 == 0:
            sumC4 += 1

wall = -1

for i in range((N//2)-1,(N//4)-1,-1):# ищем количество нулевых  элементов в четных столбцах в области 1
    wall += 1
    for j in range(1+wall , (N//2)-wall-1 ):
        
        if j < i and j > ((N//2) - 1) - i and C[j][i]==0 and  i % 2 != 0:
            sumC1 += 1
